...
def split_by_Lat(bin_, df):
    ...
    fluxlowlat = np.array(df_lat.loc[(df_lat['Latitude'] > -5) & (df_lat['Latitude'] <5), 'flux'].dropna().reset_index(drop=True))
    fluxhighlat = np.array(df_lat.loc[(df_lat['Latitude'] <= -5) | (df_lat['Latitude'] >= 5), 'flux'].dropna().reset_index(drop=True))
