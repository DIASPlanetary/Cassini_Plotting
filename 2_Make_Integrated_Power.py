...
def make_df(year):
    ...
    #Load SKR data.
    if year == 2017:
        file_data = input_data_fp + '/SKR_2017_001-258_CJ.sav'
    else: 
        file_data = input_data_fp + '/SKR_{}_CJ.sav'.format(year)
    ...
    traj_fp = output_data_fp + '/interpedtrajectory{}.csv'.format(year)
...
for i in range(2004, 2018, 1):
    df = make_df(i)
    fp = output_data_fp + '/intpwr_withtraj{}.csv'.format(i)
    df.to_csv(fp, index=False)
    list_intpwr_files.append(df)
    print(i)
        
ttl_dfs = pd.concat(list_intpwr_files)
ttl_dfs.to_csv(output_data_fp + '/intpwr_withtraj_2004_2017.csv', index=False)
